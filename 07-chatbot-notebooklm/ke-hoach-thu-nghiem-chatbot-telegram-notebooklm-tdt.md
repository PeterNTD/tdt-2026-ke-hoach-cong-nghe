# KẾ HOẠCH THỬ NGHIỆM CHATBOT TELEGRAM KẾT NỐI NOTEBOOKLM CHO PROJECT TDT 2026

## 1. Mục tiêu

Xây dựng bản thử nghiệm chatbot trên Telegram để truy cập và hỏi đáp kho dữ liệu TDT 2026 đã được tạo riêng trên NotebookLM.
...
- NotebookLM có trả lời tốt trên bộ tài liệu TDT hay không.
...
→ Gửi câu hỏi đến NotebookLM notebook TDT
→ NotebookLM trả lời dựa trên nguồn tài liệu
...
| Python handler | Xử lý tin nhắn, gọi NotebookLM CLI/API, định dạng kết quả. |
| `notebooklm-py` | Công cụ kết nối NotebookLM. |
| NotebookLM notebook TDT | Kho dữ liệu đã nạp tài liệu nguồn. |
| Log thử nghiệm | Lưu câu hỏi, thời gian, trạng thái, lỗi, đánh giá chất lượng. |

### 3.3. Lệnh NotebookLM dự kiến
...
- Không tự ý upload thêm tài liệu mới lên NotebookLM nếu chưa được phê duyệt.
...
| 2 | Gọi NotebookLM | Gửi câu hỏi vào notebook TDT và nhận câu trả lời. | Cần làm |
...
| 6 | Timeout/fallback | Nếu NotebookLM lỗi hoặc quá lâu, bot báo lỗi rõ ràng. | Cần làm |
...
| 3 | Lệnh `/status` | Kiểm tra trạng thái bot và NotebookLM auth. |
...
- Nếu NotebookLM có citation thì giữ lại phần nguồn/chỉ dẫn tài liệu.
...
- Tóm tắt ngắn gọn.
- Không bịa nội dung nếu NotebookLM không tìm thấy nguồn.
- Nếu chưa nạp file QĐ vào NotebookLM thì báo cần đồng bộ nguồn trước.
...
|---|---|
| Kết nối | Bot nhận được câu hỏi Telegram và gọi được NotebookLM. |
...
| Ổn định | Có timeout/fallback khi NotebookLM lỗi hoặc phản hồi chậm. |
...
1. Kiểm tra lại NotebookLM auth.
...
```text
tdt-2026/
  07-chatbot-notebooklm/
    ke-hoach-thu-nghiem-chatbot-telegram-notebooklm-tdt.md
    config.example.json
    chatbot_telegram_notebooklm.py
    logs/
      .gitkeep
    tests/
      cau-hoi-mau.md
...
```json
{
  ...
  "logging": {
    "enabled": true,
    "log_file": "tdt-2026/07-chatbot-notebooklm/logs/chatbot.log"
  }
}
```
...
Prompt định hướng nên dùng trước khi gửi câu hỏi vào NotebookLM hoặc khi xử lý câu trả lời:

```text
Bạn là chatbot hỗ trợ tra cứu kế hoạch công nghệ TDT 2026. 
Chỉ trả lời dựa trên tài liệu nguồn trong NotebookLM. 
...
|---|---|---|
| NotebookLM mất phiên đăng nhập | Bot không trả lời được | Có lệnh kiểm tra auth; hướng dẫn login lại. |
| NotebookLM trả lời thiếu nguồn | Giảm độ tin cậy | Yêu cầu bot nói rõ khi thiếu nguồn; test câu hỏi mẫu. |
| Dữ liệu NotebookLM chưa cập nhật | Trả lời theo tài liệu cũ | Có quy trình đồng bộ source trước khi test. |
...
| Phụ thuộc dịch vụ bên ngoài | Gián đoạn khi Google/NotebookLM lỗi | Có fallback báo lỗi; cân nhắc RAG nội bộ giai đoạn sau. |

---

## 13. Kết luận đề xuất

Nên triển khai bản thử nghiệm Telegram chatbot kết nối NotebookLM theo phạm vi nhỏ trước, chỉ cho user được phép sử dụng. Nếu kết quả hỏi đáp đạt yêu cầu, có thể mở rộng thành web chat nội bộ hoặc chuyển sang hệ thống RAG riêng để chủ động hơn về dữ liệu, phân quyền và vận hành lâu dài.
